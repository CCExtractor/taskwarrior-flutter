...
class BurnDownDailyReplica extends StatelessWidget {
  BurnDownDailyReplica({super.key});
  ...
  Map<String, Map<String, int>> _processData(List<TaskForReplica> tasks) {
    debugPrint(
        'Processing ${tasks.length} tasks for daily burndown chart (Replica).');
    Map<String, Map<String, int>> dailyInfo = {};

    // Sort tasks by modified date in ascending order
    tasks.sort((a, b) => (a.modified ?? 0).compareTo(b.modified ?? 0));

    for (var task in tasks) {
      // Use 'modified' timestamp (seconds since epoch)
      final int? modifiedTimestamp = task.modified;
      ...
      final String date = Utils.formatDate(modifiedDate.toLocal(), 'MM-dd');
        ...
        return Column(
          ...
          children: [
            ...
            const CommonChartIndicator(
              title: 'Daily Burndown Chart (Replica)',
            ),
          ],
        );
      },
